...
public class DnsimpleContext {
	...
	public DnsimpleContext(String email, String apiKey, String url) {
		...
		this.headers[1] = new BasicHeader("X-DNSimple-Token", email + ":"
				+ apiKey);
		this.headers[2] = new BasicHeader("Content-Type",
	...
	public DnsimpleContext(String domainToken, String url) {
		...
		this.headers = new Header[3];
		this.headers[0] = new BasicHeader("Accept", "application/json");
		this.headers[1] = new BasicHeader("X-DNSimple-Domain-Token",
				domainToken);
		this.headers[2] = new BasicHeader("Content-Type",
				"application/json; charset=" + CHARSET);
	}
		...
		try {
			...
			if (entity != null) {
				...
				Reader reader = new InputStreamReader(entity.getContent(),
						CHARSET);
				LinkedList<HashMap<String, Domain>> list = gson.fromJson(
						reader, collectionType);
				for (HashMap<String, Domain> map : list) {
					// There should only be one entry in the map, but this is
					// neater...
	 ...
	 * 
	 * @param domain
	 *            the name or the ID of the domain to get
	 ...
	 * 
	 * @param domain
	 *            the name or the ID of the domain to check
	 * @return {@code true} if <strong>domain</strong> is available,
	...
	public boolean isDomainAvailable(String domain) {
		...
		try {
			...
			if (statusCode == HttpStatus.SC_NOT_FOUND) {
				...
				// May need to make a more flexible exception for cases such as
				// this
				throw new UnexpectedResponseException(HttpStatus.SC_OK,
						statusCode);
			}
		} catch (IOException e) {
			throw new RuntimeIOException(e);
	 ...
	 * 
	 * @param domain
	 *            the name or the ID of the domain to enable auto renewal for
	 ...
	 * 
	 * @param domain
	 *            the name or the ID of the domain to disable auto renewal for
	 * @return details for the domain
	 * @throws UnexpectedResponseException
	 *             If the HTTP response code from DNSimple's API was not what
	 *             was expected
	 ...
	 * 
	 * @param domain
	 *            the name or the ID of the domain to set the nameservers for
	 * @param nameServers
	 *            maximum of 6 name servers
	 * @return {@code true} for success, {@code false} otherwise
	 * @throws IllegalStateException
	 *             If more than 6 name servers are given
	 * @throws UnexpectedResponseException
	 *             If the HTTP response code from DNSimple's API was not what
	 *             was expected
	 * @throws IOException
	...
	public boolean setNameServers(String domain, Collection<String> nameServers) {
		...
		if (nameServers.size() > 6) {
			throw new IllegalStateException(
					"Maximum of 6 name servers supported. Number given: "
							+ nameServers.size());
		}
	 ...
	 * 
	 * @param domain
	 *            the name or the ID of the domain to get records for
	...
	public List<Record> getRecords(String domain) {
		...
		try {
			...
			if (entity != null) {
				...
				Reader reader = new InputStreamReader(entity.getContent(),
						CHARSET);
				LinkedList<HashMap<String, Record>> list = gson.fromJson(
						reader, collectionType);
	 ...
	 * Running the above example twice would fail with a 422 response code. If
	 * the IP is changed then a new record will be created, but with the same
	 * name. Only changing the TTL will not create a new record. This described
	 * behaviour is undocumented so may change at any time.
	 * <p>
	 ...
	 * 
	 * @param domain
	 *            the name or the ID of the domain to set the record for
	 * @param record
	 *            the record to set
	 * @return the newly created record with all fields populated (domain ID,
	 *         record ID, created at, updated at etc.)
	 * @throws UnexpectedResponseException
	 *             If the HTTP response code from DNSimple's API was not what
	 *             was expected
	 * @throws IOException
	 ...
	 * 
	 * @param domain
	 *            the name or the ID of the domain to update the record for
	 * @param recordId
	 *            the ID of the record to be updated
	 * @param record
	 *            what the record should be updated to
